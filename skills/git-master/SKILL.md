...
## Local Commit Convention

In this workflow, commit messages must use a Conventional Commits style prefix such as `fix:`, `feat:`, `docs:`, `chore:`, `refactor:`, `test:`, or `ci:`.

When recent repository history conflicts with this rule, keep the atomic grouping and safety checks from this skill, but prefer the prefixed commit message format required by this workflow.
...
1. Detect message style from recent history. Use the dominant local pattern, language, and casing. In this workflow, the final commit message must still use the prefixed format from Local Commit Convention.
...
7. Commit with the required prefixed style. After each commit, verify `git log -1 --oneline`.
