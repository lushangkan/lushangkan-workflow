...
- **aaddrick/attention-control**：同源于 i-have-adhd 和 ASD-STE100 的英文输出规范（航空管制纪律类比）。贡献 B8（禁语清单）、D8（收尾动作）、A4 的"给查证命令"半句、C10 的习语删除。它的"永远给具体时间估算"不采纳（同 A3 取舍）；STE 英文语法层不移植；其评测管线（24 case 盲评加发布门）留作将来回归测试的方法论参照。调研全文见 `docs/research/2026-08-20-attention-control.md`。
